...
if exists(select * from sys.tables t join sys.schemas s on t.schema_id = s.schema_id where s.name = 'dbo' and t.name = 'RegularTable') drop table dbo.RegularTable;
if exists(select * from sys.partition_schemes where name = 'psRegularTable') drop partition scheme psRegularTable;
if exists(select * from sys.partition_functions where name = 'pfRegularTable') drop partition function pfRegularTable;
go
...
-- You can perform online index rebuild in SQL Server 2014+
create unique clustered index IDX_RegularTable_OrderDate_OrderId
on dbo.RegularTable(OrderDate, OrderId)
