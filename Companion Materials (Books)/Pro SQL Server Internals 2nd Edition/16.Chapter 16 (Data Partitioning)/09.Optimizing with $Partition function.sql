...
if exists(select * from sys.views v join sys.schemas s on v.schema_id = s.schema_id where s.name = 'dbo' and v.name = 'Data') drop view dbo.Data;
if exists(select * from sys.tables t join sys.schemas s on t.schema_id = s.schema_id where t.name = 'Data' and s.name = 'dbo') drop table dbo.Data;
if exists(select * from sys.partition_schemes where name = 'psData2') drop partition scheme psData2;
if exists(select * from sys.partition_functions where name = 'pfData2') drop partition function pfData2;
go
...
select @LastDateModified = '2016-05-25'
