...
if exists(select * from sys.tables t join sys.schemas s on t.schema_id = s.schema_id where s.name = 'dbo' and t.name = 'CustomersWithOptionalSSN') drop table dbo.CustomersWithOptionalSSN;
go
...
/* Uniqueness of the optional values */
create table dbo.CustomersWithOptionalSSN
(
...
create unique index IDX_CustomersWithOptionalSSN_SSN on dbo.CustomersWithOptionalSSN(SSN)
where SSN is not null;
go

insert into dbo.CustomersWithOptionalSSN(CustomerId,SSN)
values(1,null),(2,'111-11-1111');
go

-- Success
insert into dbo.CustomersWithOptionalSSN(CustomerId,SSN) values(3,null);
go

-- Error
insert into dbo.CustomersWithOptionalSSN(CustomerId,SSN) values(4,'111-11-1111');
go
