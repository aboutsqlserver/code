...
-- Transaction has not been automatically rolled back if you run script multiple times
if @@trancount > 0 
	rollback;

if exists(select * from sys.procedures p join sys.schemas s on p.schema_id = s.schema_id where s.name = 'dbo' and p.name = 'AtomicBlockDemo') drop proc dbo.AtomicBlockDemo;
if exists(select * from sys.tables t join sys.schemas s on t.schema_id = s.schema_id where s.name = 'dbo' and t.name = 'MOData') drop table dbo.MOData;
go
...
begin tran
	...
	select @@TRANCOUNT as [@@TRANCOUNT], XACT_STATE() as [XACT_STATE()];
commit
select * from dbo.MOData with (snapshot);
go
