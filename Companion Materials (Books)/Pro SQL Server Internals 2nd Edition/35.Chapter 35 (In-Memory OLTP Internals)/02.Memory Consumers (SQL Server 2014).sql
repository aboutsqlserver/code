...
if exists(select * from sys.tables t join sys.schemas s on t.schema_id = s.schema_id where s.name = 'dbo' and t.name = 'MemoryConsumers') drop table dbo.MemoryConsumers;
go
...
(
	...
	Name varchar(256) collate Latin1_General_100_BIN2 not null,
	index IDX_Name nonclustered(Name)
)
with (memory_optimized=on, durability=schema_only);

select 
	i.name as [Index], i.index_id
...
from 
	sys.dm_db_xtp_memory_consumers c left outer join sys.indexes i on
