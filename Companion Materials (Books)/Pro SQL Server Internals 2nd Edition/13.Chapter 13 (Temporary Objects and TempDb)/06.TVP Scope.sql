...
if exists(select * from sys.procedures p join sys.schemas s on p.schema_id = s.schema_id where s.name = 'dbo' and p.name = 'TvpDemo') drop proc dbo.TvpDemo;
if exists(select * from sys.types t join sys.schemas s on t.schema_id = s.schema_id	where s.name = 'dbo' and t.name = 'tvpErrors') drop type dbo.tvpErrors;
go
