...
if exists(select * from sys.tables t join sys.schemas s on t.schema_id = s.schema_id where s.name = 'dbo' and t.name = 'CCI') drop table dbo.CCI;
go
...
/*** Adding Rows that benefit from Page Compression */
;with N1(C) as (select 0 union all select 0) -- 2 rows
,N2(C) as (select 0 from N1 as T1 cross join N1 as T2) -- 4 rows
,N3(C) as (select 0 from N2 as T1 cross join N2 as T2) -- 16 rows
,N4(C) as (select 0 from N3 as T1 cross join N3 as T2) -- 256 rows
...
where is_allocated = 1 and allocation_unit_type_desc <> 'LOB_DATA'; 
go
...
-- In SQL Server 2014 the delta store is page compressed. 
-- In SQL Server 2016 the delta store is uncompressed. 
-- Replace PageId below
dbcc page
(
	'SQLServerInternals' 
	,3	-- FileId
...
where ip.object_id = object_id(N'dbo.CCI');
*/