...
if object_id(N'dbo.ShouldUpdateData','FN') is not null
	drop function dbo.ShouldUpdateData
go

if object_id(N'dbo.ShouldUpdateDataSchemaBound','FN') is not null
	drop function dbo.ShouldUpdateDataSchemaBound
go
...
(
	...
	where
		s.name = 'dbo' and t.name = 'HalloweenProtection'    
)
	drop table dbo.HalloweenProtection
go
