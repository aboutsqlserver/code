...
(
	...
	where s.name = 'dbo' and t.name = 'CustomersOnDisk'
)
	drop table dbo.CustomersOnDisk
go
...
(
	...
	from sys.tables t join sys.schemas s on 
		t.schema_id = s.schema_id 
	where s.name = 'dbo' and t.name = 'CustomersMemoryOptimized'
)
	drop table dbo.CustomersMemoryOptimized
go
