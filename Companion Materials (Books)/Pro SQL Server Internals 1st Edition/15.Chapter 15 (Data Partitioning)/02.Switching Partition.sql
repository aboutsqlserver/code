...
if exists(
	...
	where
		t.name = 'MainData' and s.name = 'dbo'
)
...
if exists(
	...
	from sys.tables t join sys.schemas s on
		t.schema_id = s.schema_id
	where
		t.name = 'StagingData' and s.name = 'dbo'
)
...
if exists(
	...
	where name = 'psMainData'
)
	drop partition scheme psMainData 
go
...
if exists(
	...
	where name = 'pfMainData'
)
	drop partition function pfMainData
go
