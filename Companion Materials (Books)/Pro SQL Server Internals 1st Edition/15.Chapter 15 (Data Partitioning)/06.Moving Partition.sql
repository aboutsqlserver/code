...
if exists(
	...
	where
		v.name = 'OrderData' and s.name = 'dbo'
)
	drop view dbo.OrderData
go
...
if exists(
	...
	where
		t.name = 'tblOrderData' and s.name = 'dbo'
)
	drop table dbo.tblOrderData
go
...
if exists(
	...
	from sys.tables t join sys.schemas s on
		t.schema_id = s.schema_id
	where
		t.name = 'tblOrderDataStage' and s.name = 'dbo'
)
	drop table dbo.tblOrderDataStage
go
...
if exists(
	...
	where name = 'psOrderData'
)
	drop partition scheme psOrderData 
go
...
if exists(
	...
	where name = 'pfOrderData'
)
	drop partition function pfOrderData
go
...
to (FG2014,FG2014,FG2014,FG2014,FASTSTORAGE,FASTSTORAGE,FASTSTORAGE);
...
as
	...
	from
		dbo.tblOrderData t join inserted i on
			t.OrderId = i.OrderId
	where
		i.OrderDate < '2014-05-01' or 
		i.OrderDate >= '2014-06-01'
