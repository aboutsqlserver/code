...
(
	...
	where
		s.name = 'dbo' and v.name = 'vProductSaleStats'    
)
	drop view dbo.vProductSaleStats
go
...
(
	...
	where
		s.name = 'dbo' and t.name = 'OrderLineItems09'    
)
	drop table dbo.OrderLineItems09
go
...
(
	...
	from sys.tables t join sys.schemas s on
		t.schema_id = s.schema_id
	where
		s.name = 'dbo' and t.name = 'Products'    
)
...
create table dbo.OrderLineItems09
(
...
on dbo.OrderLineItems09(ProductId);
go
...
,N5(C) as (select 0 from N4 as T1 cross join N4 as T2 ) -- 65,536 rows
,IDs(ID) as (select row_number() over (order by (select null)) from N5)
insert into dbo.OrderLineItems09(OrderId,Quantity,Price,ProductId)
...
from dbo.OrderLineItems09 o join dbo.Products p on
