...
(
	...
	where
		s.name = 'dbo' and t.name = 'InputData'    
)
...
(
	...
	from sys.tables t join sys.schemas s on
		t.schema_id = s.schema_id
	where
		s.name = 'dbo' and t.name = 'NonPersistedColumn'    
)
	drop table dbo.NonPersistedColumn
go
...
(
	...
	from sys.tables t join sys.schemas s on
		t.schema_id = s.schema_id
	where
		s.name = 'dbo' and t.name = 'PersistedColumn'    
)
	drop table dbo.PersistedColumn
go

if object_id(N'dbo.SameWithID','FN') is not null
	drop function dbo.SameWithID
go
